...
const usage = [
    "mcoda qa-tasks --input <path/to/tasks-or-review.md> [--tasks TASK-1,TASK-2] [--decision pass|fail] [--job-id <id>] [--checkpoint <path>] [--token-usage <path>] [--runs <path>] [--resume] [--out .mcoda/qa/qa-<name>.md] [--overwrite]",
    "",
    "Marks selected tasks with a QA result and writes a QA log for prototyping.",
].join("\n");
...
const applyQa = (tasks, targetIds, decision) => {
    ...
    const updated = tasks.map((task) => {
        ...
        if (!allowedStatuses.has(task.status)) {
            skipped.push({
                ...
                title: task.title,
                status: task.status,
                reason: `Status gating: expected ready_to_qa, found ${task.status || "unknown"}`,
            });
            return task;
        }
        const from = task.status || "ready_to_qa";
        let to = "completed";
        let comment = "QA passed in helper.";
        if (decision === "fail") {
            to = "in_progress";
            comment = "QA failed in helper; returned to in_progress.";
        }
...
const main = async () => {
    ...
    const { updated, outcomes, skipped, missing } = applyQa(tasks, options.targetIds, options.decision);
    ...
    // eslint-disable-next-line no-console
    console.log(`QA log written to ${options.outputPath}`);
};
main().catch((error) => {
    // eslint-disable-next-line no-console
