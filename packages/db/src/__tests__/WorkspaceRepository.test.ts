...
const withTempHome = async <T>(fn: (home: string) => Promise<T>): Promise<T> => {
  ...
  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mcoda-db-home-"));
...
test("WorkspaceRepository createProjectIfMissing is idempotent", { concurrency: false }, async () => {
...
test("WorkspaceRepository getProjectById returns matching row", { concurrency: false }, async () => {
  await withTempHome(async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mcoda-db-"));
    const repo = await WorkspaceRepository.create(dir);
    try {
...
test("WorkspaceRepository tryAcquireTaskLock overrides stale locks", { concurrency: false }, async () => {
  await withTempHome(async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mcoda-db-"));
    const repo = await WorkspaceRepository.create(dir);
    try {
...
test("WorkspaceRepository cleanupExpiredTaskLocks removes expired locks", { concurrency: false }, async () => {
  await withTempHome(async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mcoda-db-"));
    const repo = await WorkspaceRepository.create(dir);
    try {
...
test("WorkspaceRepository releaseTaskLocksByJob clears locks for a job", { concurrency: false }, async () => {
  await withTempHome(async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mcoda-db-"));
    const repo = await WorkspaceRepository.create(dir);
    try {
...
test("WorkspaceRepository resolves and reopens task comments by slug", { concurrency: false }, async () => {
  await withTempHome(async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mcoda-db-"));
    const repo = await WorkspaceRepository.create(dir);
    try {
