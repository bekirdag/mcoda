...
"CONTEXT NOTES:",
  ...
  "- If required implementation files do not exist yet, explicitly plan to create them and include those new paths in TARGETS.",
  "- In PLAN steps, include method/object-level responsibilities (what each function/module will do).",
  "- For each existing TARGET file, include at least one PLAN step naming that exact file and stating what will change there and what will be added there.",
  ...
  "- Every PLAN line must include request-specific nouns (feature/domain terms from the user request) and concrete implementation detail.",
  "- Every VERIFY line must include request-specific nouns plus a concrete check target (file/module/route/endpoint/screen).",
  "- TARGETS must be concrete repo-relative paths. Do NOT output placeholders like \"path/to/file.ts\" or \"unknown\".",
...
export const ARCHITECT_VALIDATE_PROMPT = [
  ...
  "- Ensure every PLAN and VERIFY line includes request-specific nouns and concrete targets/checks.",
  "- Ensure TARGETS are concrete repo-relative paths (no placeholders or \"unknown\").",
  "- Do NOT output implementation code.",
