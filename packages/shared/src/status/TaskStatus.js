...
export const normalizeReviewStatuses = (statuses) => {
    return Array.from(new Set((statuses ?? [])
        .map((status) => status.toLowerCase().trim())
        .filter((status) => Boolean(status) && status !== "blocked" && status !== "ready_to_review")));
};
export const filterTaskStatuses = (input, allowed, fallback) => {
    const allowedSet = new Set(allowed.map((status) => normalizeStatus(status)).filter(Boolean));
    ...
    if (filtered.length === 0) {
        return { filtered: fallback.map((status) => normalizeStatus(status)).filter(Boolean), rejected };
    }
    return { filtered, rejected };
};
export const isReadyToReviewStatus = (status) => {
    if (!status)
        return false;
