...
3. **SDK alignment and versioning**
   ...
   - Bump root and all ten release package manifests to `0.1.94` and refresh the
...
5. **Controlled release**
   - Commit the validated patch, create annotated tag `v0.1.94`, and push main
     plus the tag.
   - Monitor the release workflow to completion.
   - Verify all ten intended npm packages report `0.1.94` with internal
     dependencies pinned to `0.1.94`.
...
- GitHub release workflow for `v0.1.94` succeeds.
- npm reports `0.1.94` for all ten release packages.
