...
`create-tasks` runs preflight in SDS apply mode and updates SDS sources with resolved decisions/remediation summaries before planning context assembly.
If preflight still reports required open questions, blocking issues, or failed SDS quality gates, `create-tasks` stops before backlog generation.
The preflight-generated docs are merged into planning context before epic/story/task generation.
Use `--sds-preflight-commit` to auto-commit SDS updates generated by preflight, and `--sds-preflight-commit-message "<text>"` to override the commit message.
