...
mcoda project-guidance --workspace-root . --project WEB
```
...
- `<workspace-dir>/docs/project-guidance.md` (or `<workspace-dir>/docs/projects/<project>/project-guidance.md`) for agent guidance loaded by work/review/QA commands.

`set-workspace` bootstraps project guidance automatically. You can regenerate it manually with `mcoda project-guidance` (optionally `--project <KEY>` for project-scoped guidance).
...
- `mcoda docs pdr generate` and `mcoda docs sds generate` default to strict mode: `--quality build-ready`, `--resolve-open-questions`, `--no-placeholders`, and `--no-maybes` are on by default. You can relax boolean strictness with `--resolve-open-questions=false`, `--no-placeholders=false`, or `--no-maybes=false`.
...
- `--no-placeholders` and `--no-maybes` enforce placeholder/indecisive language removal (also implied by strict PDR/SDS defaults).
...
```sh
mcoda sds-preflight --workspace-root . --project WEB --sds <workspace-dir>/docs/sds/web.md
...
- Optional scope: `--project <KEY>` targets project-scoped planning context; omit it to use workspace-wide docs/context resolution.
...
At the beginning of `create-tasks`, mcoda runs an SDS preflight stage and writes:
- `<workspace-dir>/tasks/<PROJECT>/sds-preflight-report.json`
- `<workspace-dir>/tasks/<PROJECT>/sds-open-questions-answers.md`
- `<workspace-dir>/tasks/<PROJECT>/sds-gap-remediation-addendum.md`

`create-tasks` runs preflight in sidecar mode by default. It does not mutate SDS source files unless you explicitly opt into writeback with `--sds-preflight-apply`.
The preflight-generated docs are merged into planning context before epic/story/task generation in the default sidecar flow.
If preflight reports remediable warnings (for example unresolved questions or SDS quality gaps that were converted into sidecar guidance), `create-tasks` continues with that remediation context and records the warning/report paths in job artifacts.
If preflight itself fails to execute, `create-tasks` stops before backlog generation.
Use `--sds-preflight-apply` to update SDS sources during `create-tasks`. Use `--sds-preflight-commit` together with `--sds-preflight-apply` to auto-commit the applied SDS updates, and `--sds-preflight-commit-message "<text>"` to override the commit message.
...
Sufficiency defaults to full closure (`minCoverageRatio=1.0`) unless explicitly overridden.
If the sufficiency pass does not reach the configured target, `create-tasks` stops and reports the audit artifact path.
...
### Run SDS preflight directly
Run preflight without generating backlog items:

```sh
mcoda sds-preflight \
  --workspace-root . \
  --project TODO \
  --sds docs/sds/todo.md \
  --apply
```

- Repeat `--sds <path>` to pass multiple SDS sources.
- If `--project` is omitted, mcoda resolves it from workspace config, then derives it from workspace directory name.
- Use `--json` for machine-readable output.
- Use `--apply` to update SDS files with resolved decisions and remediation blocks.
- Use `--commit` (optionally `--commit-message`) to commit applied SDS updates.
- Artifacts are written under `<workspace-dir>/tasks/<PROJECT>/`.
...
- The audit fails fast when SDS extraction yields zero actionable implementation headings/folder signals (to prevent false-perfect coverage).
...
### Agent usage limits
Inspect tracked usage windows, reset times, and reset precision:

```sh
mcoda agent limits
mcoda agent limits --agent codex-cli
mcoda agent limits --agent codex-cli --json
```

- `RESET` shows the effective reset timestamp used for scheduling.
- `EXACT` is `yes` when reset time came from provider data, `no` when estimated fallback logic was used.
- `SOURCE` shows how reset time was derived (for example `absolute`, `relative`, `header`, or `estimated_window_fallback`).
...
- Scopes: `--project <KEY>` (optional; defaults to workspace config project, then first workspace project), `--task <KEY>...`, `--epic <KEY>`, `--story <KEY>`, default `--status ready_to_code_review` (override with `--status ...`), optional `--limit <N>`.
- Behavior: `--base <BRANCH>` (diff base), `--dry-run` (skip status transitions), `--resume <JOB_ID>`, `--agent <NAME>`, `--agent-stream <true|false>` (default false), `--execution-context-policy <best_effort|require_any|require_sds_or_openapi>`, `--empty-diff-approval-policy <ready_to_qa|complete>`, `--rate-agents`, `--json`.
...
- Invalid JSON after retry fails the task with `review_invalid_output`. Empty-diff approvals are policy-driven: `ready_to_qa` (default) keeps QA in the loop; `complete` can mark review-approved no-change tasks as completed.
...
- Scopes: `--project <KEY>` (optional; defaults to workspace config project, then first workspace project), `--task <KEY>...`, `--epic <KEY>`, `--story <KEY>`, default `--status ready_to_qa` (override for regression runs).
...
- Pipeline controls: `--dependency-policy <enforce|ignore>` controls whether QA selection enforces task dependencies. `--no-changes-policy <require_qa|skip|manual>` controls how QA handles tasks approved by review with no code changes.
